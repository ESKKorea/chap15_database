...
//                    enrollmentMenu();
    ...
    private static void registerStudent() {
        ...
        try {
            ...
            // SQL 문장 실행
            int i = pstmt.executeUpdate(); // insert,update,delete 는 excuteUpdate 써야함
            if (i > 3) {
    ...
    /**
     * 학생 조회
     * -학과 테이블과 학과테이블을 외부 조인하여 학생 정보를 조회한다.
     * -학생 ID,이름,학년,주소,학과 ID,학과명을 출력한다.
     * 오라클 데이터베이스는 대소문자를 가리지 않는다. 하지만 키워드는 대문자를 쓰는게 좋다.
     */
    private static void displayStudents() {
        ...
        try {
            ...
            pstmt = conn.prepareStatement(sql);
            rs = pstmt.executeQuery(); //단순 조회 쿼리는 excutequery() 메소드사용
    ...
    private static void updateStudent() {
        ...
        try {
            ...
            if (i > 0) {
                System.out.println("학생 정보 수정되었습니다.");
            } else {
    ...
    private static void displayTakes() {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                System.out.printf(studentId + "\t" + studentName + "\t" + courseId + "\t" + courseName + "\t" +
    ...
    private static void registerCourse() {
        ...
        String sql = "INSERT INTO course (course_id, name, credit , description) VALUES (?, ?, ?, ?)";
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            int rowsAffected = pstmt.executeUpdate();
            if (rowsAffected > 0) {
                System.out.println("강의가 성공적으로 등록 되었습니다.");
            } else {
    ...
    private static void displayCourses() {
        System.out.println("[등록된 코스 목록 조회]");
        String sql = "SELECT c.* , s.classroom " +
                " FROM course c " +
                " INNER JOIN class s on c.course_id=s.course_id ";
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                System.out.println(course_id +"\t " + name+"\t " + credit+"\t " + description);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    */
    private static void updateCourse(){
        ...
        System.out.print("강의 ID 를 입력하세요: ");
        String courseid = scanner.nextLine();
        System.out.print("강의 이름: ");
        String name = scanner.nextLine();
        System.out.print("새로운 학점: ");
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            if (rowsAffected > 0) {
                ...
                System.out.println("해당 강의 ID와 강의 이름을 찾을 수 없습니다.");
            }
        } catch (SQLException e) {
            if (e instanceof SQLIntegrityConstraintViolationException) {
                System.out.println("무결성 제약 조건 위반으로 수정에 실패했습니다.");
    ...
    private static void deleteCourse() {
        ...
        String courseId = scanner.nextLine();
        System.out.print("삭제할 강의 이름를 입력하세요: ");
        String name= scanner.nextLine();

        String sql = "DELETE FROM course WHERE course_id = ? AND name = ? ";
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
    ...
    private static void registerClass() {
        ...
        System.out.print("수업 강의 ID: ");
        String courseid = scanner.nextLine();

        System.out.print("수업 년도: ");
        int year = scanner.nextInt();
        scanner.nextLine();
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            int rowsAffected = pstmt.executeUpdate();
            if (rowsAffected > 0) {
                System.out.println("수업이 성공적으로 등록 되었습니다.");
            } else {
    ...
    private static void displayClass() {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                System.out.println(classId  +"\t"+ courseId+"\t" + year+"\t" + semester +"\t"+ professorId +"\t"+ classroom +"\t"+ enroll);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    private static void updateClass(){
        ...
        System.out.print("수업 ID 를 입력하세요: ");
        String classId = scanner.nextLine();
        System.out.print("강의 ID 를 입력하세요: ");
        String courseId= scanner.nextLine();
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            if (rowsAffected > 0) {
                ...
                System.out.println("해당 수업 ID와 강의 ID를 찾을 수 없습니다.");
            }
        } catch (SQLException e) {
            if (e instanceof SQLIntegrityConstraintViolationException) {
                System.out.println("무결성 제약 조건 위반으로 수정에 실패했습니다.");
    ...
    private static void deleteClass() {
        ...
        System.out.print("삭제할 수업 ID 를 입력하세요: ");
        String classId = scanner.nextLine();
        System.out.print("삭제할 강의 ID 를 입력하세요: ");
        String courseId= scanner.nextLine();

        String sql = "DELETE FROM class WHERE class_id = ? AND course_id = ? ";
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, classId);
            pstmt.setString(2, courseId);
            int rowsAffected = pstmt.executeUpdate();
    ...
    /**
     * PreparedStatement, ResultSet 객체 둘다 닫는 메소드
     */
